...
=item A string consisting of real (wall clock), user, and system times [default]
=item A string consisting in one of real (wall clock), user, or system times
...
the <GNU_Time_Format> environment variable as in the following example
